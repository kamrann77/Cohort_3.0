...
const NewProducts = ({ item }) => {
    ...
    const { cartData, productsData, setProductsData, setCartData, setCartToggle } = useContext(MyStore)
        ...
        >
            <div className="flex items-center gap-3">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-neutral-100 dark:bg-neutral-800">
                    <img src={item.images[2]} alt={item.title} className="h-full w-full object-contain" />
                </div>
                <span className="text-sm font-semibold text-indigo-600 dark:text-indigo-400">${item.price}</span>
            </div>
            <AddToCartButton
                size="sm"
                added={item.added}
